...
export default function PricingBlock() {
  ...
  return (
    <section
      id="pricing"
      className="relative min-h-screen flex items-center py-24 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#360808] via-[#1a0505] to-black overflow-hidden scroll-mt-20"
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-16 space-y-4">
          <span className="text-xs font-bold tracking-[0.3em] uppercase text-yellow-500 block">
            <AnimatedText animationType="fade" dependencyKey={`${currentLanguage}-pricing-subtitle`}>
              {t('pricing.subtitle') || "BẢNG GIÁ DỊCH VỤ"}
            </AnimatedText>
          </span>
          <h2 className="text-4xl lg:text-5xl font-black text-white uppercase tracking-tight">
            <AnimatedText animationType="slideUp" dependencyKey={`${currentLanguage}-pricing-title`}>
          ...
          {pricingRooms.map((room) => {
            ...
            return (
              <div
                key={room.id}
                className={cn(
                  "group relative bg-black/40 backdrop-blur-sm rounded-xl p-8 transition-all duration-300 hover:-translate-y-2",
                  room.recommended
                    ? "border-2 border-yellow-500 shadow-[0_0_30px_-5px_rgba(234,179,8,0.3)]"
                    : "border border-white/10 hover:border-yellow-500/50 hover:shadow-[0_20px_40px_-15px_rgba(234,179,8,0.1)]"
                )}
              >
                {/* Recommended Badge */}
                {room.recommended && (
                ...
                <div className="text-center mb-8">
                  <h3 className={cn(
                    "text-2xl font-bold uppercase tracking-wide mb-2 transition-colors",
                    room.recommended ? "text-yellow-500" : "text-white group-hover:text-yellow-500"
                  )}>
                    <AnimatedText
                      animationType="fade"
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 text-center text-[10px] font-bold uppercase tracking-wider text-white/30 mb-2">
                    <div className="text-left">{t('pricing.audience') || "Đối tượng"}</div>
                    <div>{Array.isArray(timings) ? timings[0] : ''}</div>
                ...
                <div className={cn(
                  ...
                  room.recommended ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"
                )} />
              </div>
            )
          })}
