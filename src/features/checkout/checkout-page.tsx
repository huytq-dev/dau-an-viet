...
import { ChevronLeft, X, Users, Clock, MapPin, CheckCircle, AlertCircle, Mail, Home } from "lucide-react"
...
export default function CheckoutPage() {
  ...
  const cartByTeam = cart.reduce((map, item) => {
  ...
  if (!meta) {
    return (
      <div className="h-screen flex items-center justify-center bg-gray-50">
        <p className="text-gray-400 text-sm">Không có dữ liệu đặt vé. <button onClick={() => router.push('/booking')} className="text-[#991b1b] underline">Quay lại</button></p>
      </div>
    )
  }
  ...
  return (
    ...
    <div className="h-screen overflow-hidden bg-gray-50 flex flex-col" style={{ paddingTop: '5rem' }}>
      ...
      <div className="bg-[#fef3c7] border-b border-[#fcd34d]/40 px-4 py-2 flex flex-wrap items-center gap-x-4 gap-y-1 shrink-0">
        <span className="text-sm text-[#7f1d1d] font-semibold">{meta.roomName} · {meta.time}</span>
        <span className="text-sm text-gray-600">{formatDateDisplay(meta.date)}</span>
        <span className="text-sm font-black text-[#991b1b] ml-auto">{formatPrice(meta.totalPrice)}</span>
      </div>
      ...
      <div className="flex flex-row flex-1 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="p-4 space-y-2 border-b border-gray-100">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                <span>{meta.time} · {formatDateDisplay(meta.date)}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <div className="p-4 border-b border-gray-100">
              <div className="rounded-xl bg-amber-50 border border-amber-100 p-3">
                <p className="text-xs font-bold text-amber-800 mb-1.5 uppercase tracking-wide">Lưu ý</p>
                <p className="text-sm text-amber-700 leading-relaxed">· Đối với khách hàng là học sinh sinh viên, vui lòng mang theo thẻ học sinh/sinh viên còn hiệu lực khi check-in, khách hàng sẽ được hoàn lại phần chênh lệch tương ứng với mức giá ưu đãi dành cho học sinh/sinh viên.</p>
              </div>
            </div>

            {/* Cart summary */}
            <div className="p-4">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Giỏ hàng của tôi</p>
              <div className="space-y-3">
                {Object.entries(cartByTeam).map(([teamId, items], idx) => {
                  ...
                  return (
                    <div key={teamId} className="rounded-xl overflow-hidden border border-gray-100">
                      <div className="px-3 py-2 text-xs font-bold" style={{ backgroundColor: palette.bg, color: palette.fg }}>
                        {items[0].teamName}
                      </div>
              ...
              <div className="mt-4 pt-3 border-t border-gray-200 flex justify-between items-center">
                ...
                <span className="text-lg font-black text-[#991b1b]">{formatPrice(meta.totalPrice)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* RIGHT: Form + QR */}
        <div className="hidden md:flex flex-col flex-1 min-h-0 overflow-y-auto p-4 lg:p-6 space-y-4" ref={formRef}>
          ...
          {!submitted ? (
            <>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-4">
                <div>
                  ...
                  <p className="text-gray-400 text-xs mt-0.5">Điền thông tin để xác nhận đặt chỗ</p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  />
                  {errors.name && <p className="text-xs text-rose-500 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{errors.name}</p>}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-3 border-t border-gray-100">
                  ...
                  <div className="bg-rose-50 rounded-xl p-3">
                    ...
                    <p className="font-black text-[#991b1b] text-xl">{formatPrice(meta.totalPrice)}</p>
                  </div>
                </div>
              </div>
            ...
            <>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <p className="text-xs text-gray-400 mb-4">Quét mã để chuyển khoản · Sau khi hoàn tất sẽ hiển thị xác nhận</p>
                ...
                <div className="mt-4 space-y-2 text-sm">
                  ...
                  <div className="flex justify-between items-center py-2">
                    ...
                    <span className="font-semibold text-gray-800">{form.name.toUpperCase()} {form.phone.slice(-4)}</span>
                  </div>
                </div>
                ...
                {paymentConfirmed && (
                  <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-xl">
                    <p className="text-sm text-green-800 font-semibold text-center">Đặt vé thành công! Chúng tôi sẽ liên hệ xác nhận qua SĐT.</p>
                  </div>
                )}
              </div>
            </>
        ...
        {!submitted ? (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Họ và tên <span className="text-[#991b1b]">*</span></label>
              <input type="text" placeholder="Nguyễn Văn A" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                className={cn('w-full px-4 py-3 rounded-xl border text-sm focus:outline-none', errors.name ? 'border-red-400 bg-red-50' : 'border-gray-200 bg-gray-50')} />
              {errors.name && <p className="text-xs text-red-500">{errors.name}</p>}
            </div>
            <div className="space-y-1">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Số điện thoại <span className="text-[#991b1b]">*</span></label>
              <input type="tel" placeholder="0901 234 567" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))}
                className={cn('w-full px-4 py-3 rounded-xl border text-sm focus:outline-none', errors.phone ? 'border-red-400 bg-red-50' : 'border-gray-200 bg-gray-50')} />
            ...
            <div className="grid grid-cols-2 gap-3 pt-2 border-t border-gray-100">
              ...
              <div className="bg-gray-50 rounded-xl p-3">
                ...
                <p className="font-black text-[#991b1b] text-lg">{formatPrice(meta.totalPrice)}</p>
              </div>
            </div>
            <button onClick={handleSubmit} className="w-full py-4 bg-[#991b1b] text-white font-black rounded-2xl text-sm transition-colors cursor-pointer">
              Xác nhận & Xem mã QR
            </button>
          </div>
        ) : (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="mt-4 space-y-2 text-sm">
              <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-gray-500">Ngân hàng</span><span className="font-semibold">MB Bank</span></div>
              <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-gray-500">Số TK</span><span className="font-mono font-semibold">0905316737</span></div>
              <div className="flex justify-between py-2"><span className="text-gray-500">Số tiền</span><span className="font-black text-[#991b1b]">{formatPrice(meta.totalPrice)}</span></div>
            </div>
            {!paymentConfirmed && (
              <button onClick={handleConfirmPayment} className="mt-4 w-full py-3 border-2 border-green-500 text-green-700 font-bold text-sm rounded-xl cursor-pointer">
                Tôi đã chuyển khoản thành công
              </button>
            )}
            {paymentConfirmed && (
              <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-xl text-center">
                <p className="text-sm text-green-800 font-semibold">Đặt vé thành công! Chúng tôi sẽ liên hệ qua SĐT.</p>
              </div>
            )}
          </div>
        )}
              ...
              <AnimatePresence mode="wait">
                {successStep === 1 ? (
                  ...
                  >
                    ...
                    <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Thông báo</p>
                    <h3 className="text-lg font-black text-gray-800 mb-2">Hóa đơn đã được gửi</h3>
                    <p className="text-sm text-gray-500 leading-relaxed mb-7">
                      Hóa đơn đã được gửi đến email của bạn.<br/>
                  ...
                  >
                    ...
                    <p className="text-xs font-bold text-[#991b1b] uppercase tracking-widest mb-2">Dấu Chân Việt</p>
                    <h3 className="text-lg font-black text-gray-800 mb-2 leading-snug">
                      Chúc bạn có một trải nghiệm tuyệt vời!
                    </h3>
                    <p className="text-sm text-gray-500 leading-relaxed mb-7">
                      Chúng tôi rất vui được đón tiếp bạn tại<br/>
